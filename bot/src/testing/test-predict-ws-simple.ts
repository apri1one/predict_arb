...
console.log(`URL: ${WS_URL.slice(0, 50)}...`);
...
ws.on('open', () => {
    ...
    const subscribeMsg = {
        method: 'subscribe',
        requestId: 1,
        params: ['predictOrderbook/1'], // 订阅市场 1 的订单簿
    };
    console.log(`[${elapsed}ms] 发送订阅请求:`, JSON.stringify(subscribeMsg));
...
ws.on('message', (data) => {
    ...
    try {
        ...
        // 订阅响应
        if (parsed.type === 'R') {
            console.log(`[${elapsed}ms] 订阅响应:`, JSON.stringify(parsed));
            return;
        }
...
ws.on('close', (code, reason) => {
    const elapsed = Date.now() - startTime;
    console.log(`[${elapsed}ms] 连接关闭: code=${code}, reason=${reason?.toString() || 'none'}`);
    console.log(`\n总消息数: ${messageCount}`);
    process.exit(0);
});
